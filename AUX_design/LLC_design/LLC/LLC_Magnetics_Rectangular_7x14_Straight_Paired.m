%% LLC 7x14: original spirals, four colored front/back terminal pairs
% All leads exit +X as before. Outer turn entries move to final terminal Y; all external leads are straight.
% S1/S4 start/return y=-7.65/-9.35 mm; S2/S3 y=+7.65/+9.35 mm.
% Each start/return pair has 1 mm XY copper-edge spacing.
% S1 L1 -> L2; S2 L3 -> L2; S3 L6 -> L7; S4 L8 -> L7.
% All secondaries: 4 turns, outside to inside, 0.7 mm trace.
% S2/S3 reflect in Y. Their Maxwell terminal references are reversed.
% Shared return layer copper-edge gap: 18 mm (required >=4 mm).
...
%% Area-sweep plots
outDir=fullfile(fileparts(mfilename('fullpath')),'output','secondary_straight_paired_1mm');
...
% Four independent four-turn secondaries with adjacent-layer end vias.
secondaryLayers=[1 3 6 8];
returnLayers=[2 2 7 7];
viaX=legCenter+halfX+2+P.ws*0.5e3-0.95;
...
for k=1:4
    ...
    if ismember(layer,[3 6]), q(:,2)=-q(:,2); end
    pairedY=q(end,2)-sign(q(end,2))*1.7;
    q(1:2,2)=pairedY;
    add_path(q,layer,k+1);
    target=returnLayers(k);
    add_path([q(1,:);30 pairedY],layer,k+1);
    assert(abs(pairedY-q(end,2))-P.ws*1e3>=1-1e-9,'Paired copper gap below 1 mm.');
    add_path([q(end,:);30 q(end,2)],target,k+1);
    vias=[vias;q(end,:) min(layer,target) max(layer,target)]; %#ok<AGROW>
end
returnGap=2*(halfY+2+P.ws*0.5e3)-P.ws*1e3;
assert(returnGap>=4,'Secondary return copper-edge spacing must be >=4 mm.');
fprintf('L2/L7 secondary return copper-edge gap: %.2f mm\n',returnGap);
...
plot_llc_geometry_and_layers(E,C,P,outDir,paths,layers,nets);
