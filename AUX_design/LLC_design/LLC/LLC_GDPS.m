...
spec.Vin_min   = 36;     % [V]
spec.Vin_max   = 36;    % [V]
spec.Vout      = 24;      % [V] (use ONE value unless you truly need a range)
spec.Pout_max  = 30;     % [W]
spec.Pout_vec  = [1 5 10 15 20 25 30];
...
% Input voltage used to convert Figure 1 tank gain into output voltage
Vin_for_voltage_plot = Vin_nom;
...
% Design knobs
Ln        = 5.5;    % Ln = Lm/Lr; Lm=6.5 uH when Lr=1 uH
Ln_sweep  = 4:12;   % Figure 6 nominal Ln values (one value per subplot)
Q_design  = 0.7;    % “design Q” at worst-case corner (heavy load)
fn_win    = [1.0, 2];   % controller window [fn_min fn_max]

% For current plots: choose Vin for worst current stress
Vin_for_current_plot = 36;
Vin_for_current_plot2 = 36;
%% ---------------------- DERIVED -------------------------
...
gain_to_vout = @(Gt) Gt .* kbridge .* Vin_for_voltage_plot ./ n;
...
fig1 = figure('Color','w','Name','Figure 1 (Combined) - Output voltage + ZVS boundary + Zero-crossings');
tlo1 = tiledlayout(fig1, 2, 1, 'Padding','compact','TileSpacing','compact');

% ---------- TOP: Output voltage + target line + per-load ZVS boundary ----------
axTop = nexttile(tlo1, 1); hold(axTop,'on'); grid(axTop,'on'); box(axTop,'on');
title(axTop, sprintf('Output voltage vs f_n at V_{in}=%.0f V', Vin_for_voltage_plot));
xlabel(axTop,'Normalized frequency f_n = f/f_r');
ylabel(axTop,'Output voltage V_{out} (V)');

% Gain family
for i = 1:nLoads
    Q_i = compute_Q(spec.Vout, Pvec(i));
    Vout_curve = gain_to_vout(Gt_norm(fn_vec, Q_i));
    plot(axTop, fn_vec, Vout_curve, 'LineWidth', 1.6, 'Color', cmap(i,:));
end

% Desired output-voltage reference
hVoutReq = yline(axTop, spec.Vout, 'r-', 'LineWidth', 2.0, ...
    'DisplayName', sprintf('V_{out,target}=%.1f V', spec.Vout));
...
plot(axTop, fn_pk, gain_to_vout(Gt_peak), 'ko', 'MarkerFaceColor','y', ...
    'DisplayName','Peak voltage (very light, inductive only)');

% Per-load ZVS boundary: Imag{Zin}=0 crossing closest to fn=1
boundary = nan(nLoads,2); % [fn_b, Vout_b]
for i = 1:nLoads
    Q_i = compute_Q(spec.Vout, Pvec(i));
...
legend(axTop, [hVoutReq hW1 hW2 hS1 hS2], 'Location','northeastoutside', 'Box','on');
...
% Label for desired output voltage
ax1 = gca; hold(ax1, 'on');
...
text(ax1, xr - 0.02*(xl(2)-xl(1)), spec.Vout - dy_red, ...
    sprintf('V_{out,target} = %.1f V', spec.Vout), ...
...
%% ===== FIGURE 6: OUTPUT-VOLTAGE ERROR FOR Ln +/-20% =====
fig6 = figure('Color','w','Name','Figure 6 - Output-voltage error for Ln tolerance');
tlo6 = tiledlayout(fig6, 3, 3, 'Padding','compact','TileSpacing','compact');
title(tlo6, sprintf(['Output-voltage difference between L_n +20%% and L_n -20%% ' ...
    '(V_{in}=%.0f V)'], Vin_for_voltage_plot));
xlabel(tlo6, 'Normalized frequency f_n = f/f_r');
ylabel(tlo6, '\DeltaV_{out} = V_{out}(1.2L_n) - V_{out}(0.8L_n) (V)');

nLn = numel(Ln_sweep);
cmapLoad = parula(nLoads);
ax6 = gobjects(nLn,1);
maskFn6 = fn_vec >= 1;
fn_vec6 = fn_vec(maskFn6);

for k = 1:nLn
    Ln_nom = Ln_sweep(k);
    ax6(k) = nexttile(tlo6, k);
    hold(ax6(k),'on'); grid(ax6(k),'on'); box(ax6(k),'on');

    for i = 1:nLoads
        Q_i = compute_Q(spec.Vout, Pvec(i));
        Vout_minus = gain_to_vout(llc_gt_norm(fn_vec6, 0.8*Ln_nom, Q_i));
        Vout_plus  = gain_to_vout(llc_gt_norm(fn_vec6, 1.2*Ln_nom, Q_i));

        deltaV_span = Vout_plus - Vout_minus;
        plot(ax6(k), fn_vec6, deltaV_span, '-', ...
            'LineWidth',1.1, 'Color',cmapLoad(i,:), ...
            'DisplayName',sprintf('%g W',Pvec(i)));
    end

    yline(ax6(k), 0, 'k-', 'LineWidth',1.0, 'HandleVisibility','off');
    xline(ax6(k), fn_min, 'k-.', 'HandleVisibility','off');
    xline(ax6(k), fn_max, 'k-.', 'HandleVisibility','off');
    title(ax6(k), sprintf('L_{n,nom} = %g', Ln_nom));
    xlim(ax6(k), [1, fn_vec6(end)]);

    colormap(ax6(k), cmapLoad);
    cb6k = colorbar(ax6(k), 'Location','eastoutside');
    cb6k.Label.String = 'Load power (W)';
    cb6k.Ticks = linspace(0,1,nLoads);
    cb6k.TickLabels = arrayfun(@(x) sprintf('%g',x), Pvec, ...
        'UniformOutput',false);
end
