...
C.legPitch=C.length-C.leg;      % 26 mm
C.windowBetweenLegs=C.length-2*C.leg; % 16 mm
...
%% Direct geometry leakage estimate for the square-core winding layout
% Primary and secondary windings surround different core legs.  Use the
% winding centre spacing dc in the side-by-side current-sheet equation:
% Llk,p = mu0*Np^2*lw/bw * (dc + (xp+xs)/3).
pIndex=0:E.Np-1;
sIndex=0:E.Ns-1;
pHalf=C.leg/2+P.clearance+P.wp/2+pIndex*(P.wp+P.spacing);
sHalf=C.leg/2+P.clearance+P.ws/2+sIndex*(P.ws+P.spacing);
P.primaryMLTGeometry=mean(8*pHalf);
P.secondaryMLTGeometry=mean(8*sHalf);
P.meanMLTGeometry=0.5*(P.primaryMLTGeometry+P.secondaryMLTGeometry);
P.primaryBreadthGeometry=2*(C.leg/2+P.clearance+P.wp+ ...
    (E.Np-1)*(P.wp+P.spacing));
P.secondaryBreadthGeometry=2*(C.leg/2+P.clearance+P.ws+ ...
    (E.Ns-1)*(P.ws+P.spacing));
P.effectiveBreadthGeometry=min(P.primaryBreadthGeometry, ...
    P.secondaryBreadthGeometry);
P.windingCentreSpacing=C.legPitch;
E.LlkGeometry=mu0*E.Np^2*P.meanMLTGeometry/P.effectiveBreadthGeometry* ...
    (P.windingCentreSpacing+(P.primaryRadial+P.secondaryRadial)/3);
E.LlkGeometrySecondary=E.LlkGeometry*(E.Ns/E.Np)^2;
E.CrGeometry=1/((2*pi*E.f)^2*E.LlkGeometry);
E.LnGeometry=E.Lm/E.LlkGeometry;
E.LlkAEDTReference=0.905626139e-6; % solved square-core model at 800 kHz

% Sweep the nearest P-S copper distance with all other winding dimensions
% and the 10 x 10 mm square leg held fixed.
DistanceScan.PSCopperGap_mm=2:0.1:14;
fixedLateral_mm=(C.leg+2*P.clearance+ ...
    P.primaryRadial+P.secondaryRadial)*1e3;
DistanceScan.WindingCentreSpacing_mm= ...
    DistanceScan.PSCopperGap_mm+fixedLateral_mm;
DistanceScan.CoreLength_mm=DistanceScan.WindingCentreSpacing_mm+C.leg*1e3;
DistanceScan.LlkPrimary_uH=mu0*E.Np^2*P.meanMLTGeometry/ ...
    P.effectiveBreadthGeometry.* ...
    (DistanceScan.WindingCentreSpacing_mm*1e-3+ ...
    (P.primaryRadial+P.secondaryRadial)/3)*1e6;
...
targetCentreSpacing=E.Llk/(mu0*E.Np^2*P.meanMLTGeometry/ ...
    P.effectiveBreadthGeometry)-(P.primaryRadial+P.secondaryRadial)/3;
targetPSGap=targetCentreSpacing-C.leg-2*P.clearance- ...
    P.primaryRadial-P.secondaryRadial;
targetCoreLength=targetCentreSpacing+C.leg;
...
PSCopperGapScan=windowScan*1e-3-2*P.clearance- ...
    P.primaryRadial-P.secondaryRadial;
pMeanHalfScan=LEGmm*0.5e-3+P.clearance+P.wp/2+ ...
    mean(pIndex)*(P.wp+P.spacing);
sMeanHalfScan=LEGmm*0.5e-3+P.clearance+P.ws/2+ ...
    mean(sIndex)*(P.ws+P.spacing);
meanMLTLeakageScan=0.5*(8*pMeanHalfScan+8*sMeanHalfScan);
primaryBreadthLeakageScan=LEGmm*1e-3+2*P.clearance+2*P.wp+ ...
    2*(E.Np-1)*(P.wp+P.spacing);
secondaryBreadthLeakageScan=LEGmm*1e-3+2*P.clearance+2*P.ws+ ...
    2*(E.Ns-1)*(P.ws+P.spacing);
effectiveBreadthLeakageScan=min(primaryBreadthLeakageScan, ...
    secondaryBreadthLeakageScan);
centreSpacingLeakageScan=(COREmm-LEGmm)*1e-3;
LlkGeometryScan=mu0*E.Np^2.*meanMLTLeakageScan./ ...
    effectiveBreadthLeakageScan.* ...
    (centreSpacingLeakageScan+(P.primaryRadial+P.secondaryRadial)/3);
LlkGeometryScan(PSCopperGapScan<=0)=NaN;
valid=Bscan<=M.Blimit & gScan>=0.03e-3 & gScan<=0.40e-3 & ...
      windowScan>=12 & LEGmm<Hscan & ...
      PSCopperGapScan>=P.primarySecondaryClearance;
...
fprintf(['Square-core geometry leakage: %.3f uH primary-referred; ' ...
    '%.3f uH secondary-referred; Cr %.2f nF; Ln %.2f\n'], ...
    E.LlkGeometry*1e6,E.LlkGeometrySecondary*1e6, ...
    E.CrGeometry*1e9,E.LnGeometry);
fprintf(['Geometry formula reaches %.3f uH at nearest P-S copper gap %.2f mm ' ...
    '(centre spacing %.2f mm; core length %.2f mm)\n'], ...
    E.Llk*1e6,targetPSGap*1e3,targetCentreSpacing*1e3, ...
...
%% Square-core leakage scans in one figure
figLeak=figure('Color','w','Position',[40 90 1700 700]);
tlLeak=tiledlayout(figLeak,1,2,'Padding','compact','TileSpacing','compact');
title(tlLeak,{sprintf('Square-core primary-referred leakage; Np:Ns = %d:%d', ...
    E.Np,E.Ns),'Side-by-side winding geometry formula; residual 2-D/3-D fringing is not included'});
axLeak=nexttile(tlLeak);
geometry_lines(axLeak,Scan.coreLength_mm,Scan.legSide_mm, ...
    LlkGeometryScan*1e6,'Primary-referred geometry leakage (uH)', ...
    C.length*1e3,C.leg*1e3);
yline(axLeak,E.Llk*1e6,'k--','LineWidth',1.4, ...
    'DisplayName','1 uH target');
title(axLeak,'Leakage versus square-leg side and core length');
...
yline(axDistance,E.Llk*1e6,'k--','LineWidth',1.4, ...
    'DisplayName','1 uH target');
yline(axDistance,E.LlkAEDTReference*1e6,'Color',[0.35 0.35 0.35], ...
    'LineStyle',':','LineWidth',1.5, ...
    'DisplayName',sprintf('AEDT at current geometry: %.3f uH', ...
    E.LlkAEDTReference*1e6));
xline(axDistance,P.actualPSClearance*1e3,'r--','LineWidth',1.4, ...
    'DisplayName',sprintf('Current copper gap: %.1f mm', ...
    P.actualPSClearance*1e3));
plot(axDistance,P.actualPSClearance*1e3,E.LlkGeometry*1e6,'rp', ...
    'MarkerFaceColor','r','MarkerSize',12, ...
    'DisplayName',sprintf('Current formula result: %.3f uH', ...
    E.LlkGeometry*1e6));
plot(axDistance,targetPSGap*1e3,E.Llk*1e6,'ko', ...
...
title(axDistance,{'Leakage sensitivity to primary-secondary copper distance', ...
    '10 x 10 mm square leg; other winding dimensions fixed'});
grid(axDistance,'on'); legend(axDistance,'Location','northwest');
squareLeakagePlot=fullfile(outDir,'LLC_square_leakage_combined.png');
exportgraphics(figLeak,squareLeakagePlot,'Resolution',220);
...
squareDistanceCsv=fullfile(outDir,'LLC_square_PS_distance_leakage_scan.csv');
writetable(Tdistance,squareDistanceCsv);
...
T=table(COREmm(:),LEGmm(:),AEscan(:)*1e6,LEscan(:)*1e3,VEscan(:)*1e6, ...
    ...
    LlkGeometryScan(:)*1e6,PSCopperGapScan(:)*1e3, ...
    ...
    'GeometryLeakageEstimate_uH','PSCopperGap_mm'});
...
fprintf('Square-core leakage scans: %s\n',squareLeakagePlot);
fprintf('Square-core P-S distance CSV: %s\n',squareDistanceCsv);
