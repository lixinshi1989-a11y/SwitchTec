%% LLC 7x14: four colored secondaries with paired terminals at winding outline
% S1(L1->L2), S4(L8->L7): left outline x=2.6 mm, y=0/-1.7 mm.
% S2(L3->L2), S3(L6->L7): right outline x=20.4 mm, y=0/+1.7 mm.
% Paired terminal XY copper-edge gap is 1 mm; these are NOT PCB-edge leads.
% S1 L1 -> L2; S2 L3 -> L2; S3 L6 -> L7; S4 L8 -> L7.
% All secondaries: 4 turns, outside to inside, 0.7 mm trace.
% S1/S4 exit left; S2/S3 exit right and reflect in Y. Same current circulation.
% Shared return layer different-winding copper gap: 16.422089 mm (>=4 mm).
% Layer order mirrors about the PCB; original dielectric stack is retained.
% Previous AEDT leakage value below belongs to the baseline, not this revision.
...
E.LmCalibration=6.5e-6; E.LlkTarget=1e-6;
E.LlkAEDTReference=1.073314625e-6; % solved 7x14/0.5 mm/4.6 mm model
E.Vpri=E.Vin/2;
...
% The 7 mm dimension sets the yoke/assembly height and the 14 mm dimension
% is the extrusion depth.  Core-to-PCB clearance is 0.5 mm.  The 30 mm
% overall length gives a 23 mm leg pitch and a 4.6 mm nearest P-S copper gap.
C.length=30e-3;
C.legHeight=7e-3;
C.legDepth=14e-3;
...
C.coreToPCBSurface=0.5e-3;
C.legPitch=C.length-C.legHeight;
C.windowBetweenLegs=C.length-2*C.legHeight;
C.zeroGapUheight=C.legHeight+C.PCBthickness/2+C.coreToPCBSurface;
C.leBase=2*(C.legPitch+2*(C.zeroGapUheight-C.legHeight/2));
...
Kcal=E.LmCalibration*(2*Ref.gapEachJoint+Ref.le/M.mui)/Ref.Ae;
% Retain the original physical gap; Lm is an output of the new geometry.
C.gapEachJoint=Ref.gapEachJoint;
gTotal=2*C.gapEachJoint;
C.Uheight=C.zeroGapUheight-C.gapEachJoint/2;
C.le=C.leBase-gTotal;
C.Ve=C.Ae*C.le;
...
% Vary the core/winding centre spacing while keeping the 7 x 14 mm leg,
...
% Optional fixed-Lm comparison uses this design's calculated Lm. U height
% shortens by totalGap/4,
% so le = leBase-totalGap for this two-U geometry.
Scan.LmFixed=E.Lm;
leBase=LEscan+gTotal;
gFixedLm=(Kcal*AEscan/Scan.LmFixed-leBase/M.mui)/(1-1/M.mui);
...
fprintf('Core-to-PCB surface clearance %.3f mm; fixed gap %.6f mm per joint\n', ...
    C.coreToPCBSurface*1e3,C.gapEachJoint*1e3);
...
fprintf('BASELINE ONLY - previous AEDT leakage at 800 kHz: %.3f uH; new routing not solved\n', ...
    E.LlkAEDTReference*1e6);
...
assert(gTotal>0,'Selected geometry does not require a positive air gap.');
...
%% Area-sweep plots
outDir=fullfile(fileparts(mfilename('fullpath')),'output','secondary_grouped_1mm');
...
areaPlot=fullfile(outDir,'LLC_7x14_centre_leg_Ae_scan.png');
...
area_lines(nexttile(tl2),Scan.coreLength_mm,Scan.Ae_mm2,gapEachPlot, ...
    sprintf('Required gap per joint for L_m=%.2f uH (mm)',Scan.LmFixed*1e6), ...
...
gapPlotFile=fullfile(outDir,'LLC_7x14_Ae_gap_volume_height.png');
...
title(tlLeak,{sprintf('Primary-referred leakage from the side-by-side winding formula; Np:Ns = %d:%d', ...
    E.Np,E.Ns),'7 x 14 mm rectangular leg; residual 2-D/3-D fringing is not included'});
...
yline(axDistance,E.LlkTarget*1e6,'k--','LineWidth',1.4, ...
    'DisplayName','1 uH target');
yline(axDistance,E.LlkAEDTReference*1e6,'Color',[0.35 0.35 0.35], ...
    'LineStyle',':','LineWidth',1.5, ...
    'DisplayName',sprintf('Previous routing AEDT reference: %.3f uH', ...
    E.LlkAEDTReference*1e6));
...
title(axDistance,{'Leakage sensitivity to primary-secondary copper distance', ...
    '7 x 14 mm leg; Np:Ns = 3:4; other winding dimensions fixed'});
grid(axDistance,'on'); legend(axDistance,'Location','northwest');
leakagePlot=fullfile(outDir,'LLC_7x14_leakage_combined.png');
...
title(ax2,{sprintf('Side view: 7 mm height-setting section; total %.3f mm', ...
    assembledHeight*1e3),sprintf('Core-to-PCB surface clearance %.3f mm', ...
    C.coreToPCBSurface*1e3)});
dimensionPlot=fullfile(outDir,'LLC_7x14_selected_dimensions.png');
...
csvFile=fullfile(outDir,'LLC_7x14_centre_leg_Ae_scan.csv');
...
distanceCsv=fullfile(outDir,'LLC_7x14_PS_distance_leakage_scan.csv');
...
pStart=pTemplate(1,:);
primaryInnerLeft=-legCenter-(halfX+2+P.wp*0.5e3);
primaryReturnX=primaryInnerLeft+P.wp*1e3+0.30;
pReturn=[primaryReturnX pTemplate(end,2)];
for layer=[2 3]
    q=pTemplate;
...
add_path([pReturn;-30 pReturn(2)],1,1);

% Four 4T secondaries, paired ports at the secondary outline left/right.
secondaryLayers=[1 3 6 8];
returnLayers=[2 2 7 7];
vias=[pStart 1 3;pReturn 1 3];
ports=[];
for k=1:4
    layer=secondaryLayers(k);
    left=ismember(layer,[1 8]);
    q=rect_spiral(legCenter,4,P.ws*1e3,P.spacing*1e3,halfX,halfY,~left);
    inner=halfX+2+P.ws*0.5e3;
    if left, viaX=legCenter-inner+0.95; else, viaX=legCenter+inner-0.95; end
    q(end+1,:)=[viaX q(end,2)]; %#ok<AGROW>
    if ~left, q(:,2)=-q(:,2); end
    direction=1; if left, direction=-1; end
    portX=q(2,1)+direction*P.ws*0.5e3;
    q(1,:)=[portX 0];
    add_path(q,layer,k+1);
    target=returnLayers(k);
    returnY=direction*(P.ws*1e3+1);
    routeX=portX-direction*P.ws*0.5e3;
    add_path([q(end,:);routeX q(end,2);routeX returnY;portX returnY],target,k+1);
    vias=[vias;q(end,:) min(layer,target) max(layer,target)]; %#ok<AGROW>
    ports=[ports;portX 0 layer k+1;portX returnY target k+1]; %#ok<AGROW>
    assert(abs(returnY)-P.ws*1e3>=1-1e-9,'Terminal copper-edge gap below 1 mm.');
end
fprintf('Four paired terminal XY copper-edge gaps: 1.00 mm; opposite pairs at winding outline.\n');
...
for layer=shownLayers
    ...
    for k=find(layers==layer)
        colour=[.88 .10 .08];
        if nets(k)>1, palette=lines(4); colour=palette(nets(k)-1,:); end
        q=paths{k};
    ...
    for j=find(ports(:,3)==layer)'
        plot(ax,ports(j,1),ports(j,2),'ks','MarkerSize',5,'LineWidth',1.2);
    end
...
title(tl,sprintf(['Np:Ns=%d:%d; rectangular leg %.1f x %.1f mm; ' ...
    ...
    P.wp*1e3,P.ws*1e3));
outputFile=fullfile(outDir,'LLC_7x14_layer_routing.png');
exportgraphics(f,outputFile,'Resolution',180);

% Overlay each spiral and its return layer to show the paired end terminals.
fz=figure('Color','w','Position',[80 80 1100 750]);
tz=tiledlayout(fz,2,2,'Padding','compact','TileSpacing','compact');
palette=lines(4);
for net=2:5
    az=nexttile(tz); hold(az,'on'); axis(az,'equal'); grid(az,'on');
    rectangle(az,'Position',[legCenter-halfX -halfY 2*halfX 2*halfY], ...
        'FaceColor',[.75 .75 .75]);
    for j=find(nets==net)
        q=paths{j}; style='-';
        if ismember(layers(j),[2 7]), style='--'; end
        plot(az,q(:,1),q(:,2),style,'Color',palette(net-1,:),'LineWidth',2);
    end
    pp=ports(ports(:,4)==net,:);
    plot(az,pp(:,1),pp(:,2),'ks','MarkerSize',6,'MarkerFaceColor','w');
    text(az,pp(1,1),-3.5,'1 mm edge gap','HorizontalAlignment','center','FontSize',10);
    xlim(az,[-1 24]); ylim(az,[-14 14]);
    title(az,sprintf('S%d: L%d spiral (solid) -> L%d return (dashed)', ...
        net-1,secondaryLayers(net-1),returnLayers(net-1)));
    xlabel(az,'x / mm'); ylabel(az,'y / mm');
end
title(tz,'Paired terminals at secondary winding outline; 1 mm XY copper-edge gap');
exportgraphics(fz,fullfile(outDir,'LLC_7x14_grouped_terminal_detail.png'),'Resolution',180);
