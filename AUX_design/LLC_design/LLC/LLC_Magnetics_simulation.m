...
%% DMR53 material model - local AEDT Power Ferrite fit near 1 MHz / 80 C
% AEDT model: Pv [W/m^3] = Cm*f[Hz]^X*B[T]^Y.
% Fit validity is intentionally restricted to about 0.8-2 MHz and 20-50 mT.
% At 50 mT the datasheet gives 70 mW/cm^3 at 1 MHz and 300 mW/cm^3
% at 2 MHz (same at 25/100 C), giving X=2.09954. The 1 MHz, 80 C
% B-Pv graph in the 20-50 mT interval gives local Y approximately 2.8.
% Cm is anchored to exactly 70 mW/cm^3 at 1 MHz/50 mT. Kdc=0 because
% the datasheet contains no DC-bias loss data.
M.mui=900; M.mui_tol=0.25;
M.fitTemperature=80;
M.X=log(300/70)/log(2); M.Y=2.8;
M.Cm=(70e3)/(1e6^M.X*(50e-3)^M.Y); M.Kdc=0;
M.localFitB=[20;30;40;50]*1e-3;
M.localFitPv=M.Cm*1e6^M.X*M.localFitB.^M.Y;
...
% Mean path of two U halves: two leg pitches plus four vertical centre-lines.
...
% Calibrate the simple reluctance ratio to the previous 6.5-uH design. This
% predicts the small Lm rise caused by shortening le while gap/Ae/N stay fixed.
...
fprintf('AEDT Power Ferrite local fit (80 C): Cm=%.9g, X=%.6f, Y=%.6f, Kdc=%g\n', ...
    M.Cm,M.X,M.Y,M.Kdc);
fprintf('Fit Pv at 1 MHz, B=20/30/40/50 mT: %.2f / %.2f / %.2f / %.2f mW/cm^3\n', ...
    M.localFitPv/1e3);
