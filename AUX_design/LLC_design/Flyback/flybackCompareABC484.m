function [G,T]=flybackCompareABC484(Best,E,W,M,S,outDir)
...
outputStem=sprintf('Flyback_%d_%d_%d',E.Np,E.Ns,E.Naux);
% The left-leg L6/L7 AUX placement is a 48:4:3-only design override.
% This helper is also called by the 36:3:2 comparison adapter.
use484AuxOverride=isequal([E.Np E.Ns E.Naux],[48 4 3]);
...
base=struct('Scheme',"",'CoreType',"",'Description',"",'Leg_mm',leg, ...
    ...
    'AuxTraceX_mm',nan,'AuxTurnSides_mm',[],'AuxOuterSide_mm',leg+2*W.coreCopper_mm+W.auxWidth_mm+ ...
        2*(max(W.aTurns)-1)*(W.auxWidth_mm+W.spacing_mm), ...
...
for k=1:4
    ...
    auxLayers=W.aLayers;
    if k==2 && use484AuxOverride
        % Scheme B only: AUX is 2T on L6 + 1T on L7, around the left leg.
        % Centre the AUX radial band within the primary copper-edge breadth.
        q.AuxCenter_mm=q.PrimaryCenter_mm;
        q.AuxLayer1=6; q.AuxLayer2=7; auxLayers=[6 7];
    end
    if ismember(k,[2 3 4])
        q.AuxCenteredOnPrimary=true;
        radialCenter=leg/2+W.coreCopper_mm+q.PrimaryRadialBreadth_mm/2;
        q.AuxRadialCenter_mm=radialCenter;
        q.AuxTraceX_mm=q.AuxCenter_mm-radialCenter;
        auxPitch=W.auxWidth_mm+W.spacing_mm;
        auxSides=cell(size(W.aTurns));
        for ia=1:numel(W.aTurns)
            nt=W.aTurns(ia);
            auxSides{ia}=2*(radialCenter+((0:nt-1)-(nt-1)/2)*auxPitch);
        end
        assert(all(cellfun(@min,auxSides)>leg+2*W.coreCopper_mm), ...
            'AUX centred in the primary radial band must enclose the left core leg.');
        q.AuxTurnSides_mm=unique([auxSides{:}]);
        q.AuxOuterSide_mm=max(cellfun(@max,auxSides))+W.auxWidth_mm;
        assert(max(W.aTurns)*W.auxWidth_mm+(max(W.aTurns)-1)*W.spacing_mm<=q.PrimaryRadialBreadth_mm);
        q.Description=string(q.Description)+"; AUX radial band centred in P band";
    end
    auxLength=Best.AuxLayerLength_mm; % same full-leg path length; layers vary in B
    if q.AuxCenteredOnPrimary
        referenceRadius=leg/2+W.coreCopper_mm+bp/2;
        oldCoil=8*referenceRadius*W.aTurns;
        leadBudget=max(0,Best.AuxLayerLength_mm-oldCoil);
        newCoil=cellfun(@(side) sum(4*side),auxSides);
        auxLength=newCoil+leadBudget;
    end
    q.AuxEnvelope_W=E.auxRms^2*sum(W.rho*auxLength*1e-3./ ...
        (W.auxWidth_mm*W.cu_mm(auxLayers)*1e-6))*W.FacA;
...
writetable(T,fullfile(outDir,[outputStem '_ABCD_comparison.csv']));
...
fid=fopen(fullfile(outDir,[outputStem '_ABCD_report.txt']),'w'); assert(fid>=0);
cleanup=onCleanup(@() fclose(fid));
fprintf(fid,'%d:%d:%d A/B/C/D comparison; P width fixed 0.2 mm; S breadth matched to local P\n',E.Np,E.Ns,E.Naux);
fprintf(fid,'P L2/L3: %d each series; S L6/L7/L8: %d each parallel; AUX L4/L5: %d+%d series\n', ...
    W.pTurns,W.sTurns,W.aTurns(1),W.aTurns(2));
if use484AuxOverride
    fprintf(fid,'Scheme B exception: AUX is on the left leg, L6=%dT and L7=%dT.\n',W.aTurns(1),W.aTurns(2));
end
for q=G
    ...
    fprintf(fid,'AUX core centre %.3f mm; left trace centre %.3f mm; layers L%d/L%d; ideal voltage %.2f V; copper loss %.5f W\n', ...
        q.AuxCenter_mm,q.AuxTraceX_mm,q.AuxLayer1,q.AuxLayer2,q.AuxIdealVoltage_V,q.AuxEnvelope_W);
end
...
title(tl,{sprintf('%d:%d:%d - A split-leg UU | B two-leg primary UU | C centre-wound EE | D two-leg P on L1/L2/L3',E.Np,E.Ns,E.Naux), ...
...
for k=1:4
    ...
    if q.AuxCenteredOnPrimary
        drawTurnsAtSides(ax,q.AuxCenter_mm,q.AuxTurnSides_mm,[0 .5 .2]);
        plot(ax,q.AuxTraceX_mm,0,'o','Color',[0 .5 .2],'MarkerFaceColor',[0 .5 .2]);
        text(ax,q.AuxTraceX_mm,2,sprintf('AUX X=%.2f mm',q.AuxTraceX_mm),'HorizontalAlignment','center','Color',[0 .4 .15]);
    else
    ...
    auxLane=sx; auxLayers=W.aLayers;
    if k==2 && use484AuxOverride, auxLane=px; auxLayers=[6 7]; end
    for j=auxLayers
        plot(ax,auxLane,[9-j 9-j],'Color',[0 .5 .2],'LineWidth',5);
        text(ax,mean(auxLane),9-j+.25,sprintf('AUX %dT',W.aTurns(auxLayers==j)),'HorizontalAlignment','center');
    end
...
for ia=1:numel(allAxes), allAxes(ia).Toolbar.Visible='off'; end
exportgraphics(fig,fullfile(outDir,[outputStem '_ABCD_layout.png']),'Resolution',180);
...
allAxes=findall(fig,'Type','axes');
for ia=1:numel(allAxes), allAxes(ia).Toolbar.Visible='off'; end
exportgraphics(fig,fullfile(outDir,[outputStem '_ABCD_side_sections.png']),'Resolution',180);
end
...
function drawTurnsAtSides(ax,cx,sides,color)
for side=sides
    rectangle(ax,'Position',[cx-side/2,-side/2,side,side],'EdgeColor',color,'LineWidth',1.2);
end
end
