...
NAME='Flyback_Pot_Parametric_SeparateRings'
...
variables=[('D','20mm'),('core_x','43mm'),('core_y','43mm'),('core2winding','2mm'),
 ...
 ('secondary_width','if(auto_trace_width==1,(radial_breadth-(secondary_turns-1)*(trace_spacing+secondary_transition_allowance))/secondary_turns,secondary_width_manual)'),
...
assert len(list(e.GetObjectsInGroup('Solids')))==49
e.FitAll();p.SaveAs(os.path.join(OUT,NAME+'.aedt'),True)
e.ExportModelImageToFile(os.path.join(OUT,NAME+'.png'),1500,1000,[])
with open(os.path.join(OUT,'geometry_manifest.json'),'w') as f: json.dump(dict(variables=variables,solids=list(e.GetObjectsInGroup('Solids')),unclassified=list(e.GetObjectsInGroup('Unclassified'))),f,indent=2)
