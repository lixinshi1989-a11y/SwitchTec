...
candidateTable=table(LEGmm(valid),LEmm(valid),Ae(valid)*1e6, ...
    ...
    Pprimary(valid),Psecondary(valid),Pwinding(valid),Pmagnetic(valid), ...
    ...
    'SecondaryLoss_W','WindingLoss_W','CorePlusWindingLoss_W'});
...
G.leg=24; G.depth=24; G.Uheight=36; G.targetPSClearance=10;
...
G.gTotal_mm=gTotal(ig,jg)*1e3;
G.gEach_mm=gEachJoint(ig,jg)*1e3;
G.Pcore_W=Pcore(ig,jg);
G.Pwinding_W=Pwinding(ig,jg);
G.Ptotal_W=Pmagnetic(ig,jg);
...
rectangle(ax,'Position',[0 stem+gh L a],'EdgeColor','k','LineWidth',2);
rectangle(ax,'Position',[0 gh a stem],'EdgeColor','k','LineWidth',2);
rectangle(ax,'Position',[L-a gh a stem],'EdgeColor','k','LineWidth',2);
rectangle(ax,'Position',[0 -h-gh L a],'EdgeColor','k','LineWidth',2);
rectangle(ax,'Position',[0 -stem-gh a stem],'EdgeColor','k','LineWidth',2);
rectangle(ax,'Position',[L-a -stem-gh a stem],'EdgeColor','k','LineWidth',2);
...
rectangle(ax,'Position',[xA -G.depth/2 G.length G.depth],'EdgeColor','k','LineWidth',1.8);
for q=1:2
...
rectangle(ax,'Position',[xB -G2.depth/2 G2.length G2.depth],'EdgeColor','k','LineWidth',1.8);
for q=1:2
