...
% Existing eta-based waveform is a sizing envelope, not an exact 100 W
% operating waveform: ideal secondary average equals Iout/eta.
E.IsecAverageEnvelope=0.5*E.IsecPk*E.Ddemag;
E.nominalIpk=sqrt(2*E.Pout/(E.Lm*E.fs));
E.nominalD=E.nominalIpk*E.Lm*E.fs/E.Vin;
E.nominalDdemag=E.Vin*E.nominalD/E.Vref;
E.nominalIpriRms=E.nominalIpk*sqrt(E.nominalD/3);
E.nominalIsecPk=E.nominalIpk*E.Np/E.Ns;
E.nominalIsecRms=E.nominalIsecPk*sqrt(E.nominalDdemag/3);
assert(E.D+E.Ddemag<1 && E.nominalD+E.nominalDdemag<1,'DCM assumption violated.');
assert(abs(0.5*E.nominalIsecPk*E.nominalDdemag-E.Iout)<1e-10);

%% PCB planar winding rules
W.primaryWidth_mm=0.20;
W.traceSpacing_mm=0.20;
W.primaryLayers=2; W.primaryTurnsPerLayer=18;
% Match total radial COPPER-EDGE breadth, not individual trace width.
W.primaryBreadth_mm=W.primaryTurnsPerLayer*W.primaryWidth_mm+ ...
    (W.primaryTurnsPerLayer-1)*W.traceSpacing_mm;
W.secondaryTurnsPerLayer=3;
W.secondaryWidth_mm=(W.primaryBreadth_mm- ...
    (W.secondaryTurnsPerLayer-1)*W.traceSpacing_mm)/W.secondaryTurnsPerLayer;
W.secondaryBreadth_mm=W.secondaryTurnsPerLayer*W.secondaryWidth_mm+ ...
    (W.secondaryTurnsPerLayer-1)*W.traceSpacing_mm;
assert(W.secondaryBreadth_mm<=W.primaryBreadth_mm+1e-10 && ...
    W.secondaryBreadth_mm>=.95*W.primaryBreadth_mm,'Secondary radial breadth must be 95-100% of primary.');
...
W.dielectricThickness_mm=[.346 .406 .393 .406 .406 .406 .360];
W.primaryLayerIds=[2 3]; W.secondaryLayerIds=[6 7 8];
% L8 is now a winding layer: common external bus allowance, NOT an L8 route.
W.busCopper_mm=.069;
W.busWidth_mm=ceil(W.secondaryWidth_mm*sum(W.copperThickness_mm(W.secondaryLayerIds))/ ...
    W.busCopper_mm*2)/2;
W.primaryJrms_Amm2=E.IpriRms./ ...
    (W.primaryWidth_mm*W.copperThickness_mm(W.primaryLayerIds));
W.primaryJrated_Amm2=E.IpriRatedRms./ ...
    (W.primaryWidth_mm*W.copperThickness_mm(W.primaryLayerIds));
% Same length/width: DC conductance, hence assumed current share, scales with t.
W.secondaryCurrentShare=W.copperThickness_mm(W.secondaryLayerIds)/ ...
    sum(W.copperThickness_mm(W.secondaryLayerIds));
W.secondaryCurrentPerLayer_A=E.IsecRms*W.secondaryCurrentShare;
...
W.copperTemperature_C=80; W.alphaCu=0.00393;
W.copperTempFactor=1+W.alphaCu*(W.copperTemperature_C-20);
W.FacPrimary=1.15;         % assumed Rac/Rdc, NOT a calculated proximity factor
W.FacSecondary=1.25;
...
% beta approximately 2.6. Alpha=1.5 is an assumed frequency exponent: it
% DOES affect the iGSE waveform correction, even at fixed switching frequency.
% This local fit does not model DC-bias or air-gap fringing loss.
...
% in series. L6/L7/L8 secondary spirals are in parallel, with current sharing
...
%% First-order leakage for stacked windings (scheme C; two-leg A/B require 3-D)
% Ampere-turn balanced energy estimate, referred to the primary:
% Lsigma = mu0*Np^2*(MLT/b)*integral(f(z)^2 dz), f=MMF/(Np*Ip).
% Uniform current sheets across a common radial breadth; AUX open circuit.
% Includes every dielectric and copper thickness, but excludes fringing,
% unequal radial current distributions, vias and the L8 terminal breakouts.
% Energy convention: Lpp-2*n*M+n^2*Lss; approximates short-circuit leakage
% for tight coupling. Exact Lsc=Lpp-M^2/Lss requires an inductance matrix.
% Source: TI, Planar Transformers, section X (field-energy method):
% https://e2e.ti.com/cfs-file/__key/communityserver-discussions-components-files/196/planar-transformers.pdf
[LlkStackPri,Leak]=stackedLeakageEstimate(LEGmm,W,E);
LlkStackSec=LlkStackPri/(E.Np/E.Ns)^2;
% Energy processed by a clamp at turn-off, NOT automatically dissipated loss.
ElkTurnoff=0.5*LlkStackPri*E.Ipk^2;
PlkProcessed=ElkTurnoff*E.fs;
Pprimary=E.IpriRms^2.*Rp*W.FacPrimary;
PprimaryRated=E.IpriRatedRms^2.*Rp*W.FacPrimary;
...
if isfinite(bestLoss)
    ...
    Best.Paux_W=Paux(ir,ic);
    Best.PprimaryRated_W=PprimaryRated(ir,ic);
    Best.Pwinding_W=Pwinding(ir,ic);
    Best.Pcore_W=Pcore(ir,ic);
    Best.Pmagnetic_W=Pmagnetic(ir,ic);
    Best.stackedLlkPri_uH=LlkStackPri(ir,ic)*1e6;
    Best.stackedLlkSec_uH=LlkStackSec(ir,ic)*1e6;
else
...
fprintf('Above waveform is eta-based sizing envelope: ideal secondary average %.3f A.\n', ...
    E.IsecAverageEnvelope);
fprintf('Output-consistent ideal 100 W baseline: primary RMS %.4f A, secondary RMS %.4f A.\n', ...
    E.nominalIpriRms,E.nominalIsecRms);
...
fprintf('Envelope winding loss: primary %.3f + secondary %.3f + AUX %.4f = %.3f W\n', ...
    Best.Pprimary_W,Best.Psecondary_W,Best.Paux_W,Best.Pwinding_W);
fprintf('Separate primary 0.30 A RMS rating check: %.3f W (not used in envelope sum).\n', ...
    Best.PprimaryRated_W);
fprintf('Estimated core + winding loss=%.3f W\n',Best.Pmagnetic_W);
fprintf('If stacked on SAME leg (C): leakage estimate pri %.2f uH / sec %.3f uH\n', ...
    Best.stackedLlkPri_uH,Best.stackedLlkSec_uH);
fprintf('Leakage model: radial breadth %.3f mm, MMF energy thickness %.4f mm; AUX open.\n', ...
    Leak.breadth_mm,Leak.energyThickness_mm);
fprintf('1-D estimate excludes gap fringing and routed leads/vias; not a Maxwell result.\n');
fprintf('=================================================\n');

%% Export all valid candidates
outDir=fullfile(fileparts(mfilename('fullpath')),'output','36_3_2_compact');
...
candidateTable=table(LEGmm(valid),LEmm(valid),Ae(valid)*1e6, ...
    ...
    LlkStackPri(valid)*1e6,LlkStackSec(valid)*1e6,PlkProcessed(valid), ...
    PprimaryRated(valid), ...
    ...
    'StackedLsigmaPriEstimate_uH','StackedLsigmaSecEstimate_uH','StackedClampEnergyRate_W', ...
    'PrimaryRatedCurrentLoss_W'});
...
title(tl,{sprintf('TPG33 UU Flyback: %.0f V to %.0f V / %.0f W / %.0f kHz / %.0f C', ...
    ...
    sprintf('P: L2/L3 x 18T series; S: L6/L7/L8 x 3T parallel (3 layers); J_{rms,max} P/S=%.2f/%.2f A/mm^2', ...
    W.primaryJmax_Amm2,W.secondaryJmax_Amm2)});
...
G.pcbThickness=sum(W.copperThickness_mm)+sum(W.dielectricThickness_mm);
...
G.minimumLegPitch=G.primaryOuterSide/2+G.secondaryOuterSide/2+ ...
    (W.primaryWidth_mm+W.secondaryWidth_mm)/2+G.targetPSClearance;
...
G.actualPSClearance=G.legPitch-G.primaryOuterSide/2-G.secondaryOuterSide/2- ...
    (W.primaryWidth_mm+W.secondaryWidth_mm)/2;
...
title(tl2D,{sprintf('TPG33 planar Flyback selected UU core - %.0f V to %.0f V / %.0f W / %.0f kHz', ...
    ...
    sprintf('L2/L3: 18T+18T series; L6/L7/L8: 3T||3T||3T; P-S nearest/farthest=%.1f/%.1f mm', ...
...
% Three secondary turns shown around the right leg; L6/L7/L8 are parallel.
for n=1:W.secondaryTurnsPerLayer
...
text(ax,legCx(2),G.boardY/2-3,'SECONDARY: L6/L7/L8, 3T || 3T || 3T', ...
...
fprintf('Primary/secondary outer centreline sides %.1f / %.1f mm\n', ...
...
%% Alternative B: half primary on each UU leg, L2/L3 series
% L1 is treated as the PCB top side. L2/L3 primary is therefore physically
% above L6/L7/L8 secondary on the right; both legs carry primary.
G2=G;
% Window measured between core faces; include both copper-edge clearances.
G2.primaryTurnsPerLayerPerLeg=E.Np/4;
G2.primaryWidth_mm=0.20;
G2.localBreadth_mm=(E.Np/4)*G2.primaryWidth_mm+(E.Np/4-1)*W.traceSpacing_mm;
G2.secondaryWidth_mm=(G2.localBreadth_mm-(W.secondaryTurnsPerLayer-1)*W.traceSpacing_mm)/W.secondaryTurnsPerLayer;
G2.window=2*W.coreClearance_mm+2*G2.localBreadth_mm+2; % 2 mm copper-edge separation
G2.length=2*G2.leg+G2.window; G2.legPitch=G2.length-G2.leg;
G2=lowProfileUU(G2,E,M,mu0);
G2.le=2*(G2.legPitch+2*(G2.Uheight-G2.leg/2));
G2.returnLegClearance=G2.legPitch-G2.leg/2- ...
    max(G2.primaryOuterSide/2+W.primaryWidth_mm/2, ...
        G2.secondaryOuterSide/2+W.secondaryWidth_mm/2);
...
bCoil=sum(4*(G2.leg+2*W.coreClearance_mm+G2.primaryWidth_mm+ ...
    2*(0:E.Np/4-1)*(G2.primaryWidth_mm+W.traceSpacing_mm)));
bLength=2*bCoil+(G2.legPitch+2*max(0,31-(G2.leg/2+W.coreClearance_mm+W.secondaryBreadth_mm)+G2.primaryWidth_mm/2))/2;
G2.Pprimary_W=E.IpriRms^2*sum(W.rhoCu*W.copperTempFactor*bLength*1e-3./ ...
    (G2.primaryWidth_mm*W.copperThickness_mm([2 3])*1e-6))*W.FacPrimary;
G2.Pwinding_W=G.Pwinding_W-Pprimary(ig,jg)+G2.Pprimary_W; % preliminary lead budget; final audit below
G2.Ptotal_W=G2.Pcore_W+G2.Pwinding_W;
% Copper-surface separation from L3 to L6 using the original 8-layer stack.
stackDiel_mm=W.dielectricThickness_mm;
...
title(tlCmp,{sprintf('TPG33 planar Flyback winding comparison - %.0f V to %.0f V / %.0f W / %.0f kHz', ...
    E.Vin,E.Vout,E.Pout,E.fs/1e3), ...
    sprintf('N_p:N_s=%d:%d, L_m=%.2f mH; P=L2/L3 18T+18T, S=L6/L7/L8 3T||3T||3T', ...
    E.Np,E.Ns,E.Lm*1e3), ...
...
for n=1:W.secondaryTurnsPerLayer
    side=G2.leg+2*(W.coreClearance_mm+G2.secondaryWidth_mm/2+(n-1)*(G2.secondaryWidth_mm+W.traceSpacing_mm));
    rectangle(ax,'Position',[cB(2)-side/2 -side/2 side side], ...
        'EdgeColor',[.04 .28 .80],'LineWidth',2.0);
end
for cx=cB
    for n=1:G2.primaryTurnsPerLayerPerLeg
        side=G2.leg+2*W.coreClearance_mm+G2.primaryWidth_mm+2*(n-1)*(G2.primaryWidth_mm+W.traceSpacing_mm);
        rectangle(ax,'Position',[cx-side/2 -side/2 side side],'EdgeColor',[.78 .10 .06]);
    end
end
drawAuxPlan(ax,cB(2),G2,W);
text(ax,cB(1),0,sprintf('P 9T/layer\nL2/L3'), ...
    'HorizontalAlignment','center','FontWeight','bold');
text(ax,cB(2),30,'P on both legs; S right','HorizontalAlignment','center','FontWeight','bold');
dimHfb(ax,xB,xB+G2.length,-38,sprintf('Core %.0f mm',G2.length));
xlim(ax,[-55 60]); ylim(ax,[-43 35]);
title(ax,sprintf('B - primary split over both legs\nl_e=%.0f mm, gap=%.3f mm, loss=%.2f W', ...
...
for n=W.primaryLayerIds
    y=9-n; plot(ax,[10 40],[y y],'Color',[.78 .10 .06],'LineWidth',7);
    plot(ax,[55 75],[y y],'Color',[.78 .10 .06],'LineWidth',7);
    text(ax,25,y+.2,'P 9T'); text(ax,65,y+.2,'P 9T');
end
for n=W.secondaryLayerIds
...
text(ax,55,8.7,'P left/right L2/L3; S right L6/L7/L8', ...
    'HorizontalAlignment','center','FontWeight','bold');
...
G3=G2;
G3.outerLeg=G2.leg/2; G3.yoke=G2.leg/2; G3.window=ceil((2*W.coreClearance_mm+max(W.primaryBreadth_mm,W.secondaryBreadth_mm))*2)/2;
...
% Compact redesign length estimate. Existing AEDT remains the old geometry.
% Common output connection is budgeted as an external bus since L8 is occupied.
% R=rho*l/A is a centreline estimate; routing/crowding still require validation.
pInner=G3.leg/2+W.coreClearance_mm+W.primaryWidth_mm/2;
pOuter=pInner+(W.primaryTurnsPerLayer-1)*pPitch_mm;
sInner=G3.leg/2+W.coreClearance_mm+W.secondaryWidth_mm/2;
sOuter=sInner+(W.secondaryTurnsPerLayer-1)*sPitch_mm;
pRouted_m=LpLayer(ig,jg)+(31-pInner-pOuter)*1e-3;
sRouted_m=LsLayer(ig,jg)+W.secondaryTurnsPerLayer*sPitch_mm*1e-3;
aRouted_m=auxLength_m(ig,jg)+(31-(G3.leg/2+W.auxOffset_mm)- ...
    W.auxWidth_mm-W.traceSpacing_mm)*1e-3;
busLane=W.secondaryWidth_mm/2+W.busWidth_mm/2+W.traceSpacing_mm;
surfaceLength_m=[busLane+31-(sOuter+sPitch_mm),busLane+31-sInner]*1e-3;
rho=W.rhoCu*W.copperTempFactor;
rP=rho*pRouted_m./(W.primaryWidth_mm*W.copperThickness_mm(W.primaryLayerIds)*1e-6);
rS=rho*sRouted_m./(W.secondaryWidth_mm*W.copperThickness_mm(W.secondaryLayerIds)*1e-6);
rA=rho*aRouted_m./(W.auxWidth_mm*W.copperThickness_mm(W.auxLayerIds)*1e-6);
rBus=rho*surfaceLength_m/(W.busWidth_mm*W.busCopper_mm*1e-6);
% Primary series current traverses L2-L3; the L1 via extension is a stub.
rPvia=rho*(W.dielectricThickness_mm(2)+ ...
    sum(W.copperThickness_mm([2 3]))/2)*1e-3/(pi*(.1e-3)^2);
% Provisional solid via model: I6 in L6-L7, I6+I7 in L7-L8. L8 adds its
% branch at the bus. Final plated-via/pad implementation is not yet designed.
viaArea=pi*(W.secondaryWidth_mm*1e-3/2)^2;
rSviaBranch=rho*(W.dielectricThickness_mm(6)+sum(W.copperThickness_mm([6 7]))/2)*1e-3/viaArea;
rSviaCommon=rho*(W.dielectricThickness_mm(7)+sum(W.copperThickness_mm([7 8]))/2)*1e-3/viaArea;
G3.Pprimary_W=E.IpriRms^2*(sum(rP)+rPvia)*W.FacPrimary;
G3.PprimaryRated_W=E.IpriRatedRms^2*(sum(rP)+rPvia)*W.FacPrimary;
G3.PsecondaryLayers_W=sum((E.IsecRms*W.secondaryCurrentShare).^2.*rS)*W.FacSecondary;
G3.PsecondaryBus_W=E.IsecRms^2*sum(rBus)*W.FacSecondary;
G3.PsecondaryVias_W=2*((E.IsecRms*W.secondaryCurrentShare(1))^2*rSviaBranch+ ...
    (E.IsecRms*sum(W.secondaryCurrentShare(1:2)))^2*rSviaCommon)*W.FacSecondary;
G3.Psecondary_W=G3.PsecondaryLayers_W+G3.PsecondaryBus_W+G3.PsecondaryVias_W;
G3.Paux_W=E.IauxRms^2*sum(rA)*W.FacAux;
G3.Pwinding_W=G3.Pprimary_W+G3.Psecondary_W+G3.Paux_W;
G3.PwindingNominal_W=(G3.Pprimary_W+G3.Psecondary_W)* ...
    (E.nominalIpriRms/E.IpriRms)^2+G3.Paux_W;
nominalWaveformFactor=E.nominalD^(1-M.alpha)+E.nominalDdemag^(1-M.alpha);
G3.PcoreNominal_W=G3.Pcore_W*(E.nominalIpk/E.Ipk)^M.beta* ...
    nominalWaveformFactor/waveformFactor;
G3.PtotalNominal_W=G3.PcoreNominal_W+G3.PwindingNominal_W;
G3.Ptotal_W=G3.Pcore_W+G3.Pwinding_W;
G3.returnLegClearance=G3.leg/2+G3.window- ...
    max(G3.primaryOuterSide/2+W.primaryWidth_mm/2, ...
        G3.secondaryOuterSide/2+W.secondaryWidth_mm/2);
...
fprintf('Compact EE envelope copper: P %.4f W; S layers %.4f W; external bus %.4f W; estimated vias %.6f W; AUX %.4f W.\n', ...
    G3.Pprimary_W,G3.PsecondaryLayers_W,G3.PsecondaryBus_W,G3.PsecondaryVias_W,G3.Paux_W);
fprintf('Ideal 100 W baseline core/copper/total %.4f / %.4f / %.4f W; AUX still specified RMS.\n', ...
    G3.PcoreNominal_W,G3.PwindingNominal_W,G3.PtotalNominal_W);
fprintf('Primary 0.30 A RMS rating check %.4f W. AC factors 1.15/1.25/1.15 are assumptions.\n', ...
    G3.PprimaryRated_W);
assert(all([pRouted_m sRouted_m aRouted_m surfaceLength_m]>0));
% Per-conductor table: series layers carry full current; parallel S layers
% split current; EACH external bus carries the full secondary current.
traceNames=["P_L2";"P_L3";"S_L6";"S_L7";"S_L8";"S_START_external_bus";"S_END_external_bus";"AUX_L4";"AUX_L5"];
traceWidths=[repmat(W.primaryWidth_mm,2,1);repmat(W.secondaryWidth_mm,3,1);repmat(W.busWidth_mm,2,1);repmat(W.auxWidth_mm,2,1)];
traceThickness=[W.copperThickness_mm([2 3 6 7 8])';W.busCopper_mm*[1;1];W.copperThickness_mm([4 5])'];
traceCurrents=[repmat(E.IpriRms,2,1);(E.IsecRms*W.secondaryCurrentShare)'; ...
    repmat(E.IsecRms,2,1);repmat(E.IauxRms,2,1)];
traceNominal=[repmat(E.nominalIpriRms,2,1);(E.nominalIsecRms*W.secondaryCurrentShare)'; ...
    repmat(E.nominalIsecRms,2,1);repmat(E.IauxRms,2,1)];
tracePeak=[repmat(E.Ipk,2,1);(E.IsecPk*W.secondaryCurrentShare)'; ...
    repmat(E.IsecPk,2,1);nan(2,1)]; % AUX waveform/peak unspecified
traceResistance=[rP';rS';rBus';rA'];
traceAC=[repmat(W.FacPrimary,2,1);repmat(W.FacSecondary,5,1);repmat(W.FacAux,2,1)];
traceAudit=table(traceNames,traceWidths,traceThickness,traceNominal,traceCurrents,tracePeak, ...
    traceCurrents./(traceWidths.*traceThickness),traceResistance, ...
    traceCurrents.^2.*traceResistance,traceCurrents.^2.*traceResistance.*traceAC, ...
    'VariableNames',{'Conductor','Width_mm','CopperThickness_mm','Ideal100W_RMS_A', ...
    'Envelope_RMS_A','EnvelopePeak_A','Envelope_Jrms_Amm2','Rdc80C_ohm', ...
    'EnvelopeDC_Loss_W','EnvelopeEstimatedAC_Loss_W'});
disp(traceAudit);
writetable(traceAudit,fullfile(outDir,'Flyback_TPG33_EE_trace_current_loss_audit.csv'));

%% Selected EE leakage and winding-size sensitivity
[G3.LlkPri_H,G3.LeakageModel]=stackedLeakageEstimate(G3.leg,W,E);
G3.LlkSec_H=G3.LlkPri_H/(E.Np/E.Ns)^2;
G3.leakageToLm_pct=100*G3.LlkPri_H/E.Lm;
G3.leakageEnergy_J=0.5*G3.LlkPri_H*E.Ipk^2;
G3.clampEnergyRate_W=G3.leakageEnergy_J*E.fs;
fprintf('\nScheme C leakage estimate (P series, S parallel, AUX open):\n');
fprintf('Lsigma,pri = %.3f uH; Lsigma,sec = %.4f uH; Lsigma/Lm = %.3f%%\n', ...
    G3.LlkPri_H*1e6,G3.LlkSec_H*1e6,G3.leakageToLm_pct);
fprintf('Turn-off leakage energy %.3f uJ; energy x fs = %.3f W (not added to loss).\n', ...
    G3.leakageEnergy_J*1e6,G3.clampEnergyRate_W);
selectedLeakage=table(G3.leg,G3.verticalPS_mm,Leak.breadth_mm,Leak.energyThickness_mm, ...
    G3.LlkPri_H*1e6,G3.LlkSec_H*1e6,G3.leakageToLm_pct,G3.clampEnergyRate_W, ...
    'VariableNames',{'LegSide_mm','PSCopperGap_mm','EffectiveRadialBreadth_mm', ...
    'MMFEnergyThickness_mm','LsigmaPriEstimate_uH','LsigmaSecEstimate_uH', ...
    'LsigmaToLm_pct','ClampEnergyRate_W'});
writetable(selectedLeakage,fullfile(outDir,'Flyback_TPG33_EE_leakage_estimate.csv'));
figLeak=figure('Color','w','Position',[100 100 920 520]);
axLeak=axes(figLeak); hold(axLeak,'on'); grid(axLeak,'on');
plot(axLeak,S.legSide_mm,LlkStackPri(:,1)*1e6,'LineWidth',1.6);
plot(axLeak,G3.leg,G3.LlkPri_H*1e6,'ro','MarkerFaceColor','r');
xlabel(axLeak,'Square centre-leg side (mm)');
ylabel(axLeak,'Primary-referred leakage estimate (uH)');
title(axLeak,{'Stacked C: L2/L3 primary, L6/L7/L8 secondary; AUX open', ...
    sprintf('1-D energy model; selected EE %.2f uH (%.2f%% of Lm)', ...
    G3.LlkPri_H*1e6,G3.leakageToLm_pct)});
legend(axLeak,'Fixed PCB stack; independent of core path length','Selected 24 mm EE', ...
    'Location','northwest');
exportgraphics(figLeak,fullfile(outDir,'Flyback_TPG33_stacked_leakage_estimate.png'),'Resolution',180);
...
title(tABC,{'Flyback A: split-leg UU | B: two-leg primary UU | C: centre-wound EE', ...
    '36:3:2 turns; 2.3 mH; P L2/L3; S L6/L7/L8; AUX L4+L5 series 1T+1T, 100 mA, 0.20 mm'});
...
fprintf('Matched radial builds: P %.3f mm, S %.3f mm; secondary trace %.3f mm, L6/L7/L8 parallel.\n', ...
    W.primaryBreadth_mm,W.secondaryBreadth_mm,W.secondaryWidth_mm);
fprintf('EE core length 80 -> %.1f mm; windows 16 -> %.1f mm; copper-edge clearance %.3f mm.\n', ...
    G3.length,G3.window,G3.returnLegClearance);
assert(abs(sum(W.secondaryCurrentPerLayer_A)-E.IsecRms)<1e-12);
assert(abs(sum(traceAudit.EnvelopeEstimatedAC_Loss_W)+ ...
    E.IpriRms^2*rPvia*W.FacPrimary+G3.PsecondaryVias_W-G3.Pwinding_W)<1e-9);
compactSummary=table(E.Np,E.Ns,E.Naux,E.Lm*1e3,G3.leg,G3.depth,G3.length,G3.window, ...
    G3.gEach_mm,W.primaryWidth_mm,W.secondaryWidth_mm,W.secondaryLayers, ...
    W.primaryBreadth_mm,W.secondaryBreadth_mm,G3.returnLegClearance, ...
    G3.Pcore_W,G3.Pwinding_W,G3.Ptotal_W,G3.LlkPri_H*1e6, ...
    'VariableNames',{'Np','Ns','Naux','Lm_mH','CentreLeg_mm','CoreDepth_mm', ...
    'CoreLength_mm','WindowEach_mm','GapEach_mm','PrimaryWidth_mm','SecondaryWidth_mm', ...
    'SecondaryParallelLayers','PrimaryRadialBreadth_mm','SecondaryRadialBreadth_mm', ...
    'CopperToReturnLeg_mm','CoreLoss_W','CopperLoss_W','TotalLoss_W','LeakagePriEstimate_uH'});
writetable(compactSummary,fullfile(outDir,'Flyback_36_3_2_compact_summary.csv'));
[Schemes,Comparison]=flybackCompareABC36(G3,E,W,M,outDir);
G4=Schemes(4); % D: two legs, primary L1/L2/L3 in series
G2.Pprimary_W=Schemes(2).PrimaryEnvelope_W;
G2.Pwinding_W=Schemes(2).CopperEnvelope_W; G2.Ptotal_W=Schemes(2).TotalEnvelope_W;
G2.BoardX_mm=Schemes(2).BoardX_mm; G2.BoardY_mm=Schemes(2).BoardY_mm;
G2.LlkPri_H=NaN; G2.LeakageModel=Schemes(2).LeakageModel;
save(fullfile(outDir,'Flyback_36_3_2_compact_design.mat'),'E','W','M','S','Best','G','G2','G3','G4','traceAudit','compactSummary','Schemes','Comparison');

function [Lpri,info]=stackedLeakageEstimate(leg_mm,W,E)
% Integrate squared normalized MMF through the real L1-to-L8 stack.
% Primary series layers carry Ip. Secondary parallel layers share total
% current inversely with DC resistance (same length/width => proportional t).
cu=W.copperThickness_mm; diel=W.dielectricThickness_mm;
assert(numel(diel)==numel(cu)-1 && all(cu>0) && all(diel>=0));
assert(W.primaryTurnsPerLayer*numel(W.primaryLayerIds)==E.Np);
assert(W.secondaryTurnsPerLayer==E.Ns);
assert(isempty(intersect(W.primaryLayerIds,W.secondaryLayerIds)));
delta=zeros(size(cu));
delta(W.primaryLayerIds)=W.primaryTurnsPerLayer/E.Np;
share=cu(W.secondaryLayerIds)/sum(cu(W.secondaryLayerIds));
delta(W.secondaryLayerIds)=-share;
f=0; integral_mm=0;
for k=1:numel(cu)
    next=f+delta(k);
    % Exact integral of a linearly varying MMF squared inside copper.
    integral_mm=integral_mm+cu(k)*(f^2+f*next+next^2)/3;
    f=next;
    if k<=numel(diel), integral_mm=integral_mm+diel(k)*f^2; end
end
assert(abs(f)<1e-12,'Primary/secondary ampere-turns must balance.');
pPitch=W.primaryWidth_mm+W.traceSpacing_mm;
sPitch=W.secondaryWidth_mm+W.traceSpacing_mm;
pBreadth=W.primaryTurnsPerLayer*W.primaryWidth_mm+ ...
    (W.primaryTurnsPerLayer-1)*W.traceSpacing_mm;
sBreadth=W.secondaryTurnsPerLayer*W.secondaryWidth_mm+ ...
    (W.secondaryTurnsPerLayer-1)*W.traceSpacing_mm;
% Common-sheet approximation uses the narrower radial winding band.
% This is a modelling choice, not a rigorous bound for unequal-width spirals.
breadth=min(pBreadth,sBreadth);
pMLT=4*(leg_mm+2*W.coreClearance_mm+W.primaryWidth_mm+ ...
    (W.primaryTurnsPerLayer-1)*pPitch);
sMLT=4*(leg_mm+2*W.coreClearance_mm+W.secondaryWidth_mm+ ...
    (W.secondaryTurnsPerLayer-1)*sPitch);
mlt=(pMLT+sMLT)/2;
Lpri=4*pi*1e-7*E.Np^2.*(mlt/breadth)*(integral_mm*1e-3);
info=struct('breadth_mm',breadth,'meanTurnLength_mm',mlt, ...
    'energyThickness_mm',integral_mm,'normalizedLayerAmpereTurns',delta);
end
