%% 72:6:4 planar flyback EE redesign, independent of the 36:3:2 files
% Run this script directly. Results go to output/72_6_4.
...
E.Np=72; E.Ns=6; E.Naux=4; E.Lm=2.3e-3;
E.primaryRatedRms=0.30; E.auxRms=0.100; % AUX is winding RMS, not DC output
E.Iout=E.Pout/E.Vout; E.ratio=E.Np/E.Ns;
E.VauxIdeal=E.Vout*E.Naux/E.Ns/2; % C: one EE outer leg carries half centre flux
...
W.maxJ_Amm2=12; % editable screening target; temperature rise still needs validation
W.sShare=W.cu_mm(W.sLayers)/sum(W.cu_mm(W.sLayers));
assert(W.pTurns==fix(W.pTurns) && W.aTurns==fix(W.aTurns));
...
S.secondaryWidths_mm=(W.pTurns*S.primaryWidths_mm+(W.pTurns-1)*W.spacing_mm- ...
    (W.sTurns-1)*W.spacing_mm)/W.sTurns; % match radial breadth
...
outDir=fullfile(fileparts(mfilename('fullpath')),'output','72_6_4');
if ~exist(outDir,'dir'), mkdir(outDir); end
writetable(T,fullfile(outDir,'Flyback_72_6_4_sweep.csv'));
writetable(struct2table(Best),fullfile(outDir,'Flyback_72_6_4_selected.csv'));

%% Conductor audit: common bus is external, not routed over the L8 winding
names=["Primary_L1";"Primary_L2";"Primary_L3";"Secondary_L6";"Secondary_L7";"Secondary_L8"; ...
    "Secondary_START_external_bus";"Secondary_END_external_bus";"AUX_L4";"AUX_L5"];
layer=[1;2;3;6;7;8;0;0;4;5]; % 0 = external bus, not a PCB layer
turns=[W.pTurns*ones(3,1);W.sTurns*ones(3,1);0;0;W.aTurns;W.aTurns];
width=[Best.PrimaryWidth_mm*ones(3,1);Best.SecondaryWidth_mm*ones(3,1); ...
    Best.SecondaryLeadWidth_mm*[1;1];W.auxWidth_mm*[1;1]];
thick=[W.cu_mm([1 2 3 6 7 8])';W.busCopper_mm*[1;1];W.cu_mm([4 5])'];
ir=[E.envelope.IpRms*ones(3,1);E.envelope.IsRms*W.sShare'; ...
    E.envelope.IsRms*[1;1];E.auxRms*[1;1]];
irNom=[E.nominal.IpRms*ones(3,1);E.nominal.IsRms*W.sShare'; ...
    E.nominal.IsRms*[1;1];E.auxRms*[1;1]];
pk=[E.envelope.Ipk*ones(3,1);E.envelope.IsPk*W.sShare'; ...
    E.envelope.IsPk*[1;1];nan(2,1)];
lengths=[Best.PrimaryLayerLength_mm*ones(3,1);Best.SecondaryLayerLength_mm*ones(3,1); ...
    Best.SecondaryStartLead_mm;Best.SecondaryEndLead_mm;Best.AuxLayerLength_mm*[1;1]];
res=W.rho*(lengths*1e-3)./(width.*thick*1e-6);
fac=[W.FacP*ones(3,1);W.FacS*ones(5,1);W.FacA*[1;1]];
...
writetable(audit,fullfile(outDir,'Flyback_72_6_4_trace_audit.csv'));
...
save(fullfile(outDir,'Flyback_72_6_4_design.mat'),'E','W','M','S','Best','MinimumLoss','T','audit');

%% Reproducible numerical report
report=sprintf(['72:6:4 planar EE flyback; 2200 V -> 24 V / 100 W / 100 kHz\n' ...
    'Lm target %.3f mH; ideal AUX voltage %.2f V\n' ...
    'P: L1/L2/L3 %dT each series; S: L6/L7/L8 %dT each parallel; AUX: L4/L5 %dT each on right outer leg\n' ...
    ...
    E.Lm*1e3,E.VauxIdeal,W.pTurns,W.sTurns,W.aTurns, ...
...
fid=fopen(fullfile(outDir,'Flyback_72_6_4_report.txt'),'w');
...
title(tl,'72:6:4 EE redesign - efficiency-envelope estimates');
...
exportgraphics(fig,fullfile(outDir,'Flyback_72_6_4_sweep.png'),'Resolution',170);
...
title(tl,'Selected 72:6:4 EE - winding envelopes (not connected trace routing)');
...
spec=[W.sTurns Best.SecondaryWidth_mm;W.pTurns Best.PrimaryWidth_mm;W.aTurns W.auxWidth_mm];
colors=[.1 .3 .8;.8 .1 .1;0 .5 .2];
for k=1:3
    for turn=1:spec(k,1)
        pad=2*W.coreCopper_mm+spec(k,2)+2*(turn-1)*(spec(k,2)+W.spacing_mm);
        sx=Best.Leg_mm+pad; sy=sx; cx=0;
        if k==3, sx=Best.Leg_mm/2+pad; cx=Best.CoreX_mm/2-Best.Leg_mm/4; end
...
labels={'Primary 24T (series)','Primary 24T (series)','Primary 24T (series)', ...
    'AUX 2T - right outer leg','AUX 2T - right outer leg','Secondary 6T (parallel)', ...
    'Secondary 6T (parallel)','Secondary 6T (parallel)'};
for k=1:8
...
exportgraphics(fig2,fullfile(outDir,'Flyback_72_6_4_selected_geometry.png'),'Resolution',170);

%% A/B/C comparison at the SAME selected leg, widths, turns and layer stack
% A: split-leg UU; B: same-leg UU; C: centre-wound EE.
% Each core gets its own gap solution and geometric core-loss calculation.
[Schemes,Comparison]=flybackCompareABC72(Best,E,W,M,S,outDir);
save(fullfile(outDir,'Flyback_72_6_4_design.mat'),'Schemes','Comparison','-append');
...
mu0=4*pi*1e-7;
np=W.pTurns; ns=W.sTurns; na=W.aTurns;
...
win=ceil((W.coreCopper_mm+build+W.returnCopper_mm+ba)*2)/2;
...
boardX=ceil(coreX+2*(W.coreCopper_mm+ba+W.edgeMargin_mm));
...
aCoil=sum(2*(leg/2+leg+2*(2*W.coreCopper_mm+W.auxWidth_mm+2*(0:na-1)*(W.auxWidth_mm+W.spacing_mm))));
...
aOuter=leg/2+W.coreCopper_mm+W.auxWidth_mm/2+(na-1)*(W.auxWidth_mm+W.spacing_mm);
...
ok=breadthOK && B<=S.maxB_T && gap*1e3>=S.minGapEach_mm && gap*1e3<=S.maxGapEach_mm && stem>0 && ...
    boardX<=S.maxBoardX_mm && boardY<=S.maxBoardY_mm && max([Jp Js Jl Ja])<=W.maxJ_Amm2;
R=struct('PrimaryRadialBreadth_mm',bp,'SecondaryRadialBreadth_mm',bs, ...
    ...
    'PrimaryRatedJ_Amm2',Jp,'SecondaryJ_Amm2',Js,'SecondaryLeadJ_Amm2',Jl, ...
