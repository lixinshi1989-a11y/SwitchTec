...
Independent geometry snapshot of the widened-trace D20 comparison.
"""
import os
import math
import json
...
NAME = 'Flyback_36_3_2_SquarePot_D20_Core43_100kHz'
...
# The primary's innermost turn (turn 17) is reached by a second "closing"
# arc that leaves spiral()'s own 258-282 deg gap sector at a FIXED
# radius (=inner) to land the via at mirror-safe x=0. That closing arc
# and the incoming turn16->turn17 transition arc both pass through the
# same sector; the transition's own midpoint sits only pitch/2 (~0.26 mm)
# above the closing arc's radius, well under the PWIDTH+SPACE (~0.48 mm)
# both need for clearance -> they physically overlap regardless of
# TRANSITION_ALLOWANCE (confirmed: -27 um edge spacing at (0,-12.3)mm on
# L2, i.e. exactly the innermost-turn short reported after the first fix).
# LAST_GAP_EXTRA pushes turns 0-16 out by one extra radial step so only
# the last gap (16->17) widens enough to clear the closing arc; PWIDTH is
# shrunk to keep the 9 mm breadth (and therefore AR/outer radius) fixed.
LAST_GAP_EXTRA = .5
LAST_GAP_SUBDIV = 4
...
def build():
    ...
    pEnds=[]
    for layer in [2,3]:
        seg=spiral(18,PWIDTH,Z[layer-1],last_gap_extra=LAST_GAP_EXTRA,last_gap_subdiv=LAST_GAP_SUBDIV); inner=seg[-1][1][-1]
    ...
    for index,point in enumerate(sEndpoints):
        ...
        if by>point[1]:
            ...
            lines(neck,[(point[0],point[1],BUS_Z),(point[0],by,BUS_Z)],SWIDTH,.069,blue); sParts.append(neck)
        bn='Secondary_Bus_%d'%index; term=(lane,31,BUS_Z)
        lines(bn+'_Neck',[(point[0],by,BUS_Z),(lane,by,BUS_Z)],SWIDTH,.069,blue); sParts.append(bn+'_Neck')
