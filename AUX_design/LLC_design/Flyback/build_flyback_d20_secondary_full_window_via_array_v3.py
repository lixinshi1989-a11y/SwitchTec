...
NAME = 'Flyback_D20_UniformPrimary_SecondaryFullWindow_ViaArrayV3_100kHz'
...
def build():
    ...
    # Each secondary layer is one open 352-degree annular turn.  The 9 mm
    # copper width fills the radial window from r=12 mm to r=21 mm.
    # All three layers run CCW (series-aiding).  Their 8-degree staggered
    # openings provide common terminal faces for two adjacent-layer via
    # arrays without a closed/shorted copper ring.
    sParts=[]; sEnds=[]
    rmid=R+2+SWIDTH/2
    secondary_lead_width=1.0
    layer_angles={6:(102,454),7:(94,446),8:(86,438)}
    layer_endpoints={}
    for layer in [6,7,8]:
        z=Z[layer-1]
        start_deg,end_deg=layer_angles[layer]
        seg=[]
        for j in range(12):
            pts=[]
            for u in [j/12.0,(j+.5)/12.0,(j+1)/12.0]:
                deg=start_deg+(end_deg-start_deg)*u
                pts.append(polar(rmid,math.radians(deg),z))
            seg.append(('Arc',pts))
        start=polar(rmid,math.radians(start_deg),z)
        finish=polar(rmid,math.radians(end_deg),z)
        layer_endpoints[layer]=(start,finish)
    ...
    # A full-width 9 mm lead would bridge the 8-degree opening and close
    # the L6/L8 turns.  Neck only the external leads down at the opening;
    # the useful annular winding remains 9 mm wide over the whole window.
    start=layer_endpoints[6][0]
    name='Secondary_L6_InputLead'
    term=(start[0],31,Z[5])
    lines(name,[term,start],secondary_lead_width,CU[5],blue)
    sParts.append(name);sEnds.append(term)
    # Five 1 mm vias share each transition current across the full 9 mm
    # trace width.  The first array joins the L6 finish to the L7 start;
    # the second joins the L7 finish to the L8 start.
    via_radii=[12.75,14.625,16.5,18.375,20.25]
    via_diameter=1.0
    via_angle_step=8.0
    minimum_via_clearance=2*via_radii[0]*math.sin(math.radians(via_angle_step/2))-via_diameter
    assert minimum_via_clearance>SPACE
    minimum_lead_clearance=2*(R+2)*math.sin(math.radians(via_angle_step/2))-secondary_lead_width
    assert minimum_lead_clearance>SPACE
    for tag,angle,first,last in [('L6_L7',94,6,7),('L7_L8',86,7,8)]:
        for index,rad in enumerate(via_radii):
            point=polar(rad,math.radians(angle),0)
            name='Secondary_Via_%s_%d'%(tag,index+1)
            via(name,point,first,last,via_diameter,blue)
            sParts.append(name)
    # Output remains on L8 and leaves through the top core opening.  It
    # does not pass vertically through L6/L7, so no unintended short exists.
    finish=layer_endpoints[8][1]
    name='Secondary_L8_OutputLead'
    term=(finish[0],31,Z[7])
    lines(name,[finish,term],secondary_lead_width,CU[7],blue)
    sParts.append(name);sEnds.append(term)
    ...
    with open(os.path.join(OUT,NAME+'_geometry.json'),'w') as f:
        json.dump(dict(centre_area_mm2=math.pi*R*R,centre_diameter_mm=2*R,window_radius_mm=RW,outer_side_mm=2*RO,
            ...
            secondary_external_lead_width_mm=secondary_lead_width,
            secondary_via_array=dict(count_per_transition=len(via_radii),diameter_mm=via_diameter,
                angular_separation_deg=via_angle_step,minimum_edge_clearance_mm=minimum_via_clearance,
                minimum_lead_to_opposite_end_clearance_mm=minimum_lead_clearance),
