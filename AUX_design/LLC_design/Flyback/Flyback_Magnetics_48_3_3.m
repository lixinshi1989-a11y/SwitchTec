%% 48:3:3 planar flyback EE redesign, independent of the 36:3:2 files
% Run this script directly. Results go to output/48_3_3.
...
E.Np=48; E.Ns=3; E.Naux=3; E.Lm=2.3e-3;
...
S.leg_mm=12:.5:30;
S.primaryWidths_mm=[.20 .25 .30]; % retain compact radial breadth; rated-current J is reported
...
assert(~isempty(feasible),'No feasible design: review board, gap and J limits.');
...
outDir=fullfile(fileparts(mfilename('fullpath')),'output','48_3_3');
if ~exist(outDir,'dir'), mkdir(outDir); end
writetable(T,fullfile(outDir,'Flyback_48_3_3_sweep.csv'));
writetable(struct2table(Best),fullfile(outDir,'Flyback_48_3_3_selected.csv'));
...
writetable(audit,fullfile(outDir,'Flyback_48_3_3_trace_audit.csv'));
...
save(fullfile(outDir,'Flyback_48_3_3_design.mat'),'E','W','M','S','Best','MinimumLoss','T','audit');

%% Reproducible numerical report
report=sprintf(['48:3:3 planar EE flyback; 2200 V -> 24 V / 100 W / 100 kHz\n' ...
    ...
    'Absolute minimum feasible loss %.4f W; selected compact design within %.0f %%\n' ...
...
fid=fopen(fullfile(outDir,'Flyback_48_3_3_report.txt'),'w');
assert(fid>=0); fprintf(fid,'%s',report); fclose(fid);

%% Sweep figure: loss, volume and the explicit selection trade-off
fig=figure('Color','w','Position',[50 60 1200 520]);
tl=tiledlayout(fig,1,2,'TileSpacing','compact','Padding','compact');
title(tl,'48:3:3 EE redesign - efficiency-envelope estimates');
ax=nexttile(tl); hold(ax,'on'); grid(ax,'on');
for wp=S.primaryWidths_mm
    for ws=(W.pTurns*wp+(W.pTurns-1)*W.spacing_mm-(W.sTurns-1)*W.spacing_mm)/W.sTurns
        mask=T.Feasible & T.PrimaryWidth_mm==wp & T.SecondaryWidth_mm==ws;
...
ax=nexttile(tl); hold(ax,'on'); grid(ax,'on');
scatter(ax,T.CoreVolume_cm3(T.Feasible),T.TotalEnvelope_W(T.Feasible),20,T.Leg_mm(T.Feasible),'filled');
...
exportgraphics(fig,fullfile(outDir,'Flyback_48_3_3_sweep.png'),'Resolution',170);
...
title(tl,'Selected 48:3:3 EE - winding envelopes (not connected trace routing)');
...
labels={'Primary access (routing pending)','Primary 24T (series)','Primary 24T (series)', ...
    'AUX 2T - centre leg','AUX 1T - centre leg','Secondary 3T (parallel)', ...
    'Secondary 3T (parallel)','Secondary 3T (parallel)'};
for k=1:8
...
exportgraphics(fig2,fullfile(outDir,'Flyback_48_3_3_selected_geometry.png'),'Resolution',170);

%% Matched-winding A/B/C mechanical comparison
[Schemes,Comparison]=flybackCompareABC48(Best,E,W,M,S,outDir);
save(fullfile(outDir,'Flyback_48_3_3_design.mat'),'Schemes','Comparison','-append');
...
aCoil=arrayfun(@(nt) sum(4*(leg+2*W.coreCopper_mm+W.auxWidth_mm+ ...
    2*(0:nt-1)*(W.auxWidth_mm+W.spacing_mm))),W.aTurns);
...
aOuter=leg/2+W.coreCopper_mm+W.auxWidth_mm/2+(W.aTurns-1)*(W.auxWidth_mm+W.spacing_mm);
